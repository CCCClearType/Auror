...
## Project Shape
- AurorNote is a small three-tier note-store app: static vanilla HTML/CSS/JS frontend, Go/Gin REST API, PostgreSQL database.
...
- Backend serves images at `/media/images/*` from `backend/assets/images`; note files live under `backend/assets/note-files`, are stored as `/downloads/...` URLs in `note_media`, and are only returned by `GET /api/protected/library/{note_id}/download` after license checks.
- Seller note editing lives at `frontend/pages/dashboard/edit_note.html`; `seller_dashboard.html` should only list/create notes and link to that editor.
- Note descriptions are stored in `notes.description`, exposed as JSON `desc`, edited via `PUT /api/seller/notes/:id`, and rendered as Markdown with `marked` + `DOMPurify` on detail/edit pages.
...
- `backend/main.go` also ensures the `notes.description` column exists and normalizes legacy media/download URLs on startup; check it before changing seeded media paths or note columns.
