...
### 3.2 瀏覽/搜尋商店筆記 (Browse Games Flow)
```text
browser (Frontend 進入首頁或輸入搜尋條件，JS 送出 HTTP GET ?q=Cyber&developer=DevA&hide_owned=true)
    ...
    -> go.GORM (因 hide_owned=true，額外加入條件過濾「已在 game_licenses 獲得授權的筆記」與「當前登入者自己開發的筆記」)
...
browser (Frontend 點擊「確認結帳」，JS 帶上 Token 送出 HTTP POST)
    ...
    -> go.GORM (翻譯成 INSERT INTO game_licenses 寫入筆記庫授權，狀態為 ACTIVE)
...
browser (Frontend CSR 管理員點擊「核准退款」，JS 帶上 CSR Token 送出 HTTP PUT {status: "APPROVED"})
    ...
    -> go.GORM (翻譯成 UPDATE game_licenses SET status='REVOKED' 收回筆記閱讀權限)
