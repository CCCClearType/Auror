# AurorNote 三層架構與技術流程 (System Architecture & Technical Flow)

本文件詳細解析了 AurorNote 專案的三層架構設計，並使用標準的資料流格式，深入剖析各項核心功能的實際運作流程。

---

## 1. 專案三層架構概述 (Three-Tier Architecture)

AurorNote 採用經典的現代 Web 三層架構，職責分明：
...
### 3.2 瀏覽/搜尋商店筆記 (Browse Notes Flow)
```text
browser (Frontend 進入首頁或輸入搜尋條件，JS 送出 HTTP GET ?q=Cyber&seller=DevA&hide_owned=true)
    -> RESTful API (送到 /api/notes?q=Cyber&seller=DevA&hide_owned=true)
    -> go.Gin (攔截，轉送給 NoteController 的 GetNotes 功能)
    -> go.Controller (解析 query 參數，包含關鍵字模糊搜尋、seller 精確配對，並解析隱藏已購買功能)
    -> go.GORM (翻譯成帶有 ILIKE 的 SQL，針對 seller 使用精確字串比對)
...
```text
browser (Frontend 點擊「加入購物車」，JS 從 localStorage 提取 Token，送出 HTTP POST {note_id})
    ...
    -> go.Controller (收到 note_id 與 user_id，檢查是否已經在購物車或已擁有)
    -> go.GORM (翻譯成 SELECT ... WHERE user_id=? AND note_id=? 檢查重複)
...
browser (Frontend 點擊「確認結帳」，JS 帶上 Token 送出 HTTP POST)
    ...
    -> go.GORM (翻譯成 INSERT INTO note_licenses 寫入筆記庫授權，狀態為 ACTIVE)
    ...
    -> go.Gin (打包成功訊息 json 送回去)
browser (Frontend 接收 json 資料，清空畫面並提示前往筆記庫查看)
```
...
browser (Frontend CSR 管理員點擊「核准退款」，JS 帶上 CSR Token 送出 HTTP PUT {status: "APPROVED"})
    ...
    -> go.Controller (準備更新退款單狀態與收回筆記授權)
    -> go.GORM (開啟【資料庫交易 Transaction】)
    -> go.GORM (翻譯成 UPDATE refund_requests SET status='APPROVED')
    -> go.GORM (透過 transaction_item_id 找到對應的 note_licenses)
    -> go.GORM (翻譯成 UPDATE note_licenses SET status='REVOKED' 收回筆記閱讀權限)
...
```text
browser (Frontend 偵測到使用者具備賣家權限，解鎖身分選單。送出帶有 post_as_role="AUTHOR" 的 HTTP POST)
    -> RESTful API (送到 /api/social/notes/{id}/reviews)
    -> go.Gin (攔截並通過 AuthMiddleware 解析身分)
    -> go.Controller (發現附帶了特權發布請求，跳過購買 note_licenses 的擁有權檢查)
