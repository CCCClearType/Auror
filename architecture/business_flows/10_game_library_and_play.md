...
## 2. 下載筆記檔案 (Download Game File)

- **起點**：買家在筆記庫中，針對特定筆記點擊「下載」按鈕。
- **流程 (`GET /api/protected/library/:game_id/download`)**：
  1. **授權驗證**：後端嚴格檢查 `game_licenses` 中，該買家對該 `game_id` 是否擁有 `status = 'ACTIVE'` 的紀錄。若無授權，直接回傳 HTTP 403 Forbidden。
...
- **流程 (`GET /api/protected/library/:game_id/play`)**：
  1. **授權驗證**：如同下載功能，後端嚴格檢查 `game_licenses` 中該買家對該筆記是否有 `ACTIVE` 狀態的授權。
