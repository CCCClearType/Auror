...
- **起點**：買家在 `note_detail.html` 底部查看評論 (前端呼叫 `GET /api/notes/:id/reviews`)，並點擊「寫評論」。
- **流程**：
  1. 呼叫 `POST /api/social/notes/:id/reviews`，傳入 `attitude` (`POSITIVE` 或 `NEGATIVE`) 與 `content` (文字內容)。可選傳入 `post_as_role` 指定發布身分。
  2. **購買限制檢查與身分特權 (Bypass Ownership)**：
     - 若為一般會員 (預設)，後端強制檢查該買家是否在 `note_licenses` 中擁有這款筆記，且 `status = 'ACTIVE'`。**未購買或是已退款、已吊銷授權的買家無法發表一般評論**。
     - 若選擇以特權身分發布 (例如 `ADMIN`, `CSR` 或 `AUTHOR`) 且具備對應權限，後端會**略過**購買檢查。
       *(註：前端在解鎖賣家的 `AUTHOR` 選項時，運用了 `(user.id || user.user_id) == sellerId` 高包容性探測機制，確保身份不漏接)*
  3. **隱藏字首機制與寫入**：將評論寫入 `reviews` 表。如果為特權發布，後端會在文字最前方自動加入隱藏前綴 (例如 `[ROLE:ADMIN]`)，全程**不更動 Database Schema**。狀態預設為 `'VISIBLE'`。
  4. **讀取評論**：當前端呼叫 `GET /api/notes/:id/reviews`，後端會自動剔除前綴字首，並將身分轉成 `posted_as_role` 交給前端渲染專屬的身分科目 (Badge)。
  5. **觸發自動計分 (Trigger Rating Update)**：
     - 在寫入評論成功後，後端會自動呼叫 `updateNoteOverallRating(note_id)`。
     - 該函式會統計此筆記所有 `VISIBLE` 狀態的 `POSITIVE` 數量，並計算比例。
     - 根據正面評價比例，將結果四捨五入後存回 `notes.overall_rating` (滿分 5.0 的小數)。
