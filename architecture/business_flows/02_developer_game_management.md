# 2. 賣家筆記上架與管理 (Developer Game Management)

這份文件描述了具有 `DEVELOPER` 權限的帳號，如何在 AurorNote 平台上發布、管理與下架自己的筆記。

---

## 1. 筆記草稿建立 (Create Game Draft)

- **起點**：賣家在 `dev_dashboard.html` 查看自己的筆記列表 (呼叫 `GET /api/developer/games`)，並點擊「新增筆記」。
- **流程**：
  1. 輸入基本的 `Title` (標題)、`Price` (定價)、`Description` (支援 Markdown 格式的介紹)。
  2. 呼叫 `POST /api/developer/games`。
  3. 後端驗證使用者身分是否為 `DEVELOPER`，並將賣家 ID 綁定到該款筆記。
  4. 筆記建立完成，初始狀態預設為 `'DRAFT'` (草稿)，並在 `games` 資料表生成唯一 `game_id`。此狀態下筆記不會出現在商店首頁。
- **終點**：畫面上顯示「建立成功」，自動跳轉進入 `edit_game.html?id={game_id}` 的進階編輯介面，賣家可繼續上傳素材與科目。

---

## 2. 筆記素材與檔案上傳 (Upload Media & Game Files)

- **起點**：在 `edit_game.html` 內的「上傳多媒體素材」與「上傳筆記檔案」區塊。
- **流程**：
  ...
  3. 後端根據檔案類型 (`media_type`) 進行不同的實體儲存處理：
     ...
     - **筆記檔案 (`game_file`)**：**不會進行 Hash**，而是保留賣家上傳的**原始檔名**，存入專屬資料夾 `assets/game-files/{game_id}/{original_filename}`。
  4. 實體路徑會轉換成對應的虛擬路由：圖片對應至 `/media/images/{game_id}/{hash}.{ext}`，筆記檔案則對應至受保護的下載路由 `/downloads/{game_id}/{original_filename}`，並寫入 `game_media` 表。
  5. **刪除素材**：若賣家欲刪除，可呼叫 `DELETE /api/developer/games/:id/media/:media_id` 移除不要的素材。
...
- **流程**：
  ...
  2. 若無適合科目，賣家可呼叫 `POST /api/developer/tags` 創建新科目。
  3. 呼叫 `POST /api/developer/games/:id/tags` 將 `tag_id` 與 `game_id` 綁定。這會在 `game_tags` (多對多關聯表) 中新增一筆紀錄。
  4. 若需解除科目，呼叫 `DELETE /api/developer/games/:id/tags/:tag_id` 將其移除。
...
- **起點**：賣家在 `dev_dashboard.html` 看到草稿狀態的筆記，點擊「正式發佈」。
- **流程**：
...
- **流程 (自主下架 Delete)**：
  ...
  3. 通過驗證後，將該筆記的狀態 `games.status` 設為 `'TAKEN_DOWN'` (軟刪除)。
  4. **與管理員下架的差異**：賣家自主下架時，**不會**連帶撤銷買家的 `game_licenses`。這是保障消費者的基本權益，也就是「買過的買家依然可以在筆記庫中找到它並下載閱讀，只是商店不再開放新買家購買」。
